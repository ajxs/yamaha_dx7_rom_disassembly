...
.CLR_2ND_LINE_LOOP_START:
    STAA    0,x
    INX
    DECB
    BNE     .CLR_2ND_LINE_LOOP_START            ; If b > 0, loop.
    RTS
...
.INCREMENT_LOOP_COUNTER:
    INCB
    CMPB    #32
    BNE     TEST_RAM_BLOCK_LOOP_START           ; If ACCB < 32, loop.
    RTS
...
.INCREMENT_VOICE_TARGET_PITCH_PTR:
    LDX     <VA_VOICE_PITCH_TARGET_PTR
    ABX
    STX     <VA_VOICE_PITCH_TARGET_PTR
    DEC     VA_VOICE_INDEX
...
VOICE_REMOVE_MONO_FIND_HIGHEST_KEY:
    DECB
    BNE     .IS_VOICE_EVENT_CLEAR?
    RTS

.IS_VOICE_EVENT_CLEAR?:
    LDAA    0,x
    BEQ     .INCREMENT
...
.INCREMENT:                                     ; Restore iterator.
    LDAB    <M_KEY_EVENT_CURRENT
    INCB
    CMPB    #16
    BNE     .LOOP_START                         ; If B < 16, loop.
...
.DECREMENT_LOOP_INDEX:
    DEC     $AB
    BEQ     .COPY_PATCH_VALUES                  ; If 0xAB > 0, loop.
...
.PRINT_MESSAGE:                                 ; LCD Buffer Line 2 + 9.
    LDX     #$2638
    STX     <M_COPY_DEST_PTR
    LDX     #aToOp
    JSR     LCD_WRITE_STR_TO_BUFFER
...
;
; DESCRIPTION:
; The env level goes from 0-5 to take into account the first and final levels.
;
; ==============================================================================
...
.EG_STEP_NOT_5:
    LDX     #M_PATCH_PITCH_EG_VALUES
    CMPA    #3                                  ; Clamp A at 3.
    BCS     .LOAD_PITCH_EG_RATE                 ; If A < 3, branch.
    LDAA    #3
...
; If the value goes below 0, this step is finished.
    BMI     .STORE_NEXT_LEVEL

.IS_PITCH_EG_LEVEL_LOWER_FINISHED?:
    CMPA    <M_PITCH_EG_NEXT_LVL
    BHI     .STORE_CURRENT_PITCH_EG_LEVEL
    BRA     .STORE_NEXT_LEVEL

.PITCH_EG_LEVEL_HIGHER:
    LDX     <M_PITCH_EG_VOICE_LEVELS_PTR
    LDD     0,x

; Add the pitch EG increment to the current level.
    ADDD    <M_PITCH_EG_INCREMENT
    CMPA    <M_PITCH_EG_NEXT_LVL

; If the value is still higher than the target, branch.
; Otherwise we know we're at the target level.
    BCS     .STORE_CURRENT_PITCH_EG_LEVEL
...
; If after incrementing the env step is 6, set to 0.
    CMPA    #6
    BNE     .STORE_ENV_STEP
    CLRA
...
; I'm not entirely sure under what circumstances this value with be '46'.
    BNE     .IS_BUTTON_17?
